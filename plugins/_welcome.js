...
const handler = async (m, { conn, args, isAdmin, isOwner }) => {
  if (!isAdmin &&!isOwner) return conn.reply(m.chat, `💙 𓆩 ***𝗔𝗡𝗧𝗜𝗧𝗢𝗣 𝗢𝗙𝗜𝗖𝗜𝗔𝗟*** 𓆪 💙\n\n🫐 *Solo admins pueden usar este comando*`, m)
  ...
  if (/on/i.test(args[0])) {
    ...
    await conn.reply(m.chat, `💙 𓆩 ***𝗔𝗡𝗧𝗜𝗧𝗢𝗣 𝗢𝗙𝗜𝗖𝗜𝗔𝗟*** 𓆪 💙\n\n📌 *Uso:* ${m.prefix}bienvenida on/off`, m)
  }
}
...
handler.before = async function (m, { conn, groupMetadata }) {
  ...
  const DEFAULT_IMG = 'https://files.evogb.win/OdOIUP.jpg' // imagen default azul/antitop
  let imgBuffer = null
  ...
  try {
    ...
    // PASO 2: Si falla, descargar la default de Antitop
    try {
