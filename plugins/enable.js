let handler = async (m, { conn, usedPrefix, command, args, isOwner, isAdmin, isROwner }) => {
  ...
  if (!args[0]) return m.reply(`💙 𐔌 ꒱ 𝗣𝗔𝗡𝗘𝗟 𝗔𝗡𝗧𝗜𝗧𝗢𝗣 𐔌 ꒱ 💙
...
*Ejemplo:* ${usedPrefix + command} on 🦋`)
  ...
  switch (type) {
    case 'welcome': case 'bienvenida':
      if (m.isGroup &&!isAdmin) { return conn.reply(m.chat, `🫐 *Solo admins pueden*`, m); fail = true; break }
  ...
  let statusTxt = ` 𐔌 ꒱ 𝗖𝗢𝗡𝗙𝗜𝗚𝗨𝗥𝗔𝗖𝗜𝗢𝗡 𐔌 ꒱
