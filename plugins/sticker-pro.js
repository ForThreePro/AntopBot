...
let handler = async (m, { conn, text, usedPrefix, command, args }) => {
    ...
    if (command === 'wm' || command === 'take' || command === 'robar') {
        ...
        try {
            let stiker = await addExif(img, packname || '***Antitop Bot Oficial***', author || '🪼')
            await conn.sendFile(m.chat, stiker, 'antitop.webp', '', m)
            await m.react('✅')
        } catch (e) {
            console.error(e)
            await m.react('❌')
    ...
    if (command === 's' || command === 'sticker' || command === 'stiker') {
        ...
        let stiker = await sticker(img, false, '***Antitop Bot Oficial***', '🪼')
        await conn.sendFile(m.chat, stiker, 'antitop.webp', '', m)
        await m.react('✅')
    }
    ...
    if (command === 'qc' || command === 'quotly') {
        ...
        if (mentionedJid && args.join(" ").includes("/")) {
            ...
            pp = await conn.profilePictureUrl(mentionedJid, 'image').catch(_ => 'https://files.evogb.win/OdOIUP.jpg')
        } else if (!mentionedJid && args.join(" ").includes("/")) {
            const joined = args.join(" ")
            const [authorNameRaw,...textParts] = joined.split("/")
            authorName = authorNameRaw?.trim() || "Anónimo"
            txt = textParts.join("/").trim()
            pp = "https://files.evogb.win/OdOIUP.jpg"
            ...
            pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://files.evogb.win/OdOIUP.jpg')
        ...
        try {
            ...
            const stiker = await sticker(buffer, false, '***Antitop Bot Oficial***', '🪼')

            if (stiker) {
                await conn.sendFile(m.chat, stiker, 'antitopqc.webp', '', m)
                await m.react('✅')
            } else {
                await m.react('❌')
            }
    ...
    function error(msg) {
        let texto = `🌀 *𝗔𝗡𝗧𝗜𝗧𝗢𝗣 𝗕𝗢𝗧 𝗢𝗙𝗜𝗖𝗜𝗔𝗟 - 𝗦𝗧𝗜𝗖𝗞𝗘𝗥𝗦* 🌀
