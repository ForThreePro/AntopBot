...
setInterval(async () => {
  ...
  for (let i of global.tempGroups) {
    ...
    if (now >= i.exitTime) {
      ...
      while (attempts < 3) {
        try {
          await global.conn.sendMessage(i.id, { text: '⏰ Temporizador finalizado. Saliendo del grupo... 💙' })
...
let handler = async (m, { conn, args, command }) => {
  ...
  if (command === 'templist') {
    ...
    return m.reply(`💙 𐔌 ꒱ 𝗟𝗜𝗦𝗧𝗔 𝗗𝗘 𝗧𝗘𝗠𝗣𝗢𝗥𝗜𝗭𝗔𝗗𝗢𝗥𝗘𝗦 𐔌 ꒱ 💙
│
${list}
│
.⃟𖥔 ݁. 𖦹˙— \`\`ANTITOP BOT\`\` —˙𖦹.🫧꒷`)
  }
  ...
  await m.reply(`💙 𐔌 ꒱ 𝗧𝗘𝗠𝗣𝗢𝗥𝗜𝗭𝗔𝗗𝗢𝗥 𝗔𝗖𝗧𝗜𝗩𝗔𝗗𝗢 𐔌 ꒱ 💙
│
│ 🏠 *GRUPO:* ${groupName}
...
handler.before = async (m, { conn, command }) => {
  if (command === 'tempcancel') {
    ...
    saveTempGroups()
    await m.reply(`🗑️ 𐔌 ꒱ 𝗧𝗘𝗠𝗣𝗢𝗥𝗜𝗭𝗔𝗗𝗢𝗥 𝗖𝗔𝗡𝗖𝗘𝗟𝗔𝗗𝗢 𐔌 ꒱ 🗑️
│
│ 🏠 *GRUPO:* ${groupName}
│ ✅ *ESTADO:* Cancelado
│
