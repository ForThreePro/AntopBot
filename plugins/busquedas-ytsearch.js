...
let handler = async (m, { conn, text }) => {
    ...
    if (!text) return m.reply(`💙 𐔌 ꒱ ***𝗔𝗡𝗧𝗜𝗧𝗢𝗣 𝗕𝗢𝗧 𝗢𝗙𝗜𝗖𝗜𝗔𝗟*** 𐔌 ꒱ 💙\n\n✨ *¿Qué deseas buscar en YouTube lindo?* 🫧\n📌 *Ejemplo:* ${m.prefix}ytsearch mariposas azules`)
    ...
    try {
        ...
        if (!data.data || data.data.length === 0) {
            await m.react('❌')
            return m.reply(`🫐 Ay no~ *No encontré resultados para:* ${text} 🌀`)
        }
...
🔗 ${v.url}`).join('\n\n')
        ...
        let caption = `🫧 𐔌 ꒱ 𝗬𝗢𝗨𝗧𝗨𝗕𝗘 𝗦𝗘𝗔𝗥𝗖𝗛 𐔌 ꒱ 🫧
